...
def plot_images(imgs, titles=None, cmaps="gray", dpi=100, pad=0.5, adaptive=True):
    ...
    imgs = [
        # convert to (H, W, 3) for torch.Tensor
...
def add_text(
    ...
    idx: 图表索引
    text: 要添加的文本内容
    pos: 文本的位置，以相对坐标(x, y)的形式给出，默认为(0.01, 0.99)
    fs: 文本的字体大小，默认为15
    color: 文本颜色，默认为白色("w")
    lcolor: 文本边框颜色，默认为黑色("k")，若为None，表示不添加边框
    lwidth: 文本边框宽度，默认为2
    ha: 水平对齐方式，默认为左对齐("left")
    va: 垂直对齐方式，默认为顶部对齐("top")
    """
    ax = plt.gcf().axes[idx]
    t = ax.text(
